...
<html>
  <head>
    ...
    <title>My JSP 'updatePersmission.jsp' starting page</title>
   ...
   <p class="father" onClick="menuClick(this);">${rote.pername }</p>
         <div>
             <c:forEach items="${rote.persmissionList }" var ="roteson" >
                  <p class="son"><input name="myBox" value="${ roteson.id}"' type=checkbox><a href="${roteson.perurl} " target="right_f" >${roteson.pername}</a></p>
             </c:forEach>
         </div>
   </c:forEach> 
  </body>
